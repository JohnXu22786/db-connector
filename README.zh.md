...
- **PostgreSQL** — `npm i pg`（可选 peer）。读走 `BEGIN TRANSACTION READ ONLY … ROLLBACK`；事务型写操作走 `BEGIN/COMMIT/ROLLBACK`，`VACUUM` 和并发索引操作因 PostgreSQL 不允许在事务中执行而直接运行；全部以 `$1..$n` 参数化；AbortSignal 取消通过 pg 查询取消 API 处理，并保留 TLS 与 Unix socket 传输设置。
