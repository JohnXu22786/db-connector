...
export interface ReadOutcome {
  ...
  rows: unknown[][];
  rowCount: number;
  /** True when a bounded driver stopped retaining rows after finding another. */
  truncated?: boolean;
}
...
export interface DriverApi {
   ...
   * is supplied, the driver must not materialize more than that many rows;
   * `truncated` reports whether another row was observed.
   */
  read(
    sql: string,
    params: unknown[],
